...
class TemplateRepository {
  ...
  Stream<List<Template>> watchAll() => (db.select(
    db.templates,
  ...
  Future<Template?> findById(int id) => (db.select(
    db.templates,
  )..where((t) => t.id.equals(id))).getSingleOrNull();

  /// Stream a fully resolved template (all blocks + exercises).
  Stream<ResolvedTemplate?> watchResolved(int templateId) async* {
    final templateStream = (db.select(
      db.templates,
    )..where((t) => t.id.equals(templateId))).watchSingleOrNull();
  ...
  Future<int> duplicate(int sourceId, String newName) async {
    return db.transaction(() async {
      ...
      final sourceBlocks = await (db.select(
        db.templateBlocks,
      )..where((b) => b.templateId.equals(sourceId))).get();
      for (final sb in sourceBlocks) {
        final newBlockId = await db
            .into(db.templateBlocks)
            .insert(
              TemplateBlocksCompanion.insert(
                templateId: newId,
  ...
  Future<int> getBlockRefId(int templateId, int dayId, String blockId) async {
    final row =
        await (db.select(db.templateBlocks)..where(
              (b) =>
  ...
  Future<void> addExercise(
    ...
    await db.transaction(() async {
      ...
      )..where((e) => e.blockRefId.equals(blockRefId))).get();
      await db
          .into(db.templateExercises)
          .insert(
            TemplateExercisesCompanion.insert(
  ...
  Future<void> updateExercise(
    ...
    await (db.update(
      db.templateExercises,
    )..where((e) => e.id.equals(exerciseRowId))).write(
  ...
  Future<void> removeExercise(int exerciseRowId) async {
    final row = await (db.select(
      db.templateExercises,
    )..where((e) => e.id.equals(exerciseRowId))).getSingleOrNull();
    if (row == null) return;
    await db.transaction(() async {
      await (db.delete(
        db.templateExercises,
      )..where((e) => e.id.equals(exerciseRowId))).go();
      final siblings =
          await (db.select(db.templateExercises)
      ...
      for (var i = 0; i < siblings.length; i++) {
        if (siblings[i].position != i) {
          await (db.update(db.templateExercises)
                ..where((e) => e.id.equals(siblings[i].id)))
  ...
  Future<void> reorderExercises(int blockRefId, List<int> orderedRowIds) async {
    await db.transaction(() async {
      for (var i = 0; i < orderedRowIds.length; i++) {
        await (db.update(db.templateExercises)
              ..where((e) => e.id.equals(orderedRowIds[i])))
            .write(TemplateExercisesCompanion(position: Value(i)));
      }
    });
    await _touch(blockRefId);
  ...
  Future<(Template, List<TemplateBlock>, Map<int, List<TemplateExercise>>)?>
  getTemplateForExport(int id) async {
    final template = await findById(id);
    if (template == null) return null;
    final blocks = await (db.select(
      db.templateBlocks,
    )..where((b) => b.templateId.equals(id))).get();
  ...
  Future<void> _touch(int blockRefId) async {
    final block = await (db.select(
      db.templateBlocks,
    )..where((b) => b.id.equals(blockRefId))).getSingleOrNull();
    if (block == null) return;
    await (db.update(
      db.templates,
    )..where((t) => t.id.equals(block.templateId))).write(
      TemplatesCompanion(
        updatedAt: Value(DateTime.now().millisecondsSinceEpoch),
      ),
    );
